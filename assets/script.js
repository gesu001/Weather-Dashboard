  ...
  fetch(requestUrl)
   ...
   // alert if cannot find a city
      if(data.cod !== 200){
        ...
        //console.log(data)
        //var currentDate = dayjs().format('YYY-MM-DD')
        console.log(data.dt)
      var currentDate = dayjs.unix(data.dt).format('YYYY-MM-DD');
      ...
      city.textContent = data.name + " (" + currentDate + ") "
      //console.log(iconURL)
...
function getDailyApi () {
  ...
  .then(function (data) {
    ...
    // get the next 5 days weather forcast at the time stamp of 00:00:00
...
function renderCityNames() {
  ...
  for (var i = 0; i < cityNames.length; i++) {
    ...
    liEl.setAttribute("class", "list-group-item list-group-item-action list-group-item-secondary p-2 g-col-6 my-1");
  }
}
